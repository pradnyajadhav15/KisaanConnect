...
import * as consumerApi from '../../../lib/consumerApi';
...
const normalizeProduct = (p) => ({
  id: p.id,
  name: p.name,
  price: Number(p.price_per_unit != null ? p.price_per_unit : (p.price || 0)),
  unit: p.unit,
...
export default function ConsumerDashboard() {
  ...
  const handleAddToCart = useCallback(async (product, quantity) => {
    ...
    try {
      await consumerApi.addToCart({ crop_id: product.id, quantity: qty, cart_id: cartId });
      await refreshCart();
  ...
  const handleUpdateCartItem = useCallback(async (productId, newQty) => {
    ...
    try {
      if (newQty <= 0) {
        ...
        await consumerApi.removeCartItem(cartId, existing.id);
        await consumerApi.addToCart({ crop_id: productId, quantity: newQty, cart_id: cartId });
      }
      await refreshCart();
    } catch (err) {
  ...
  const handlePlaceOrder = useCallback(async (orderDetails) => {
    ...
    if (!phone || phone.replace(/\D/g, '').length < 10) {
      notify('Please enter a valid phone number (at least 10 digits).');
    ...
    try {
      const response = await consumerApi.placeOrder({
        ...
        phone: phone,
        cart_id: cartId,
        items: cart.map((i) => ({ crop_id: i.productId, quantity: i.quantity })),
      });

      const totalAmount = response.total_amount != null
    ...
    <div className="consumer-dashboard">
      ...
      <div className="dashboard-tabs">
        {[
          ...
          >
            {tab.label}
            {tab.badge > 0 && <span className="cart-badge">{tab.badge}</span>}
          </button>
        ))}
      </div>
      ...
      <div className="dashboard-content">
        ...
        {!loading && activeTab === 'browse' && (
          <BrowseProducts products={products} onViewProduct={handleViewProduct} onAddToCart={handleAddToCart} />
        )}

        {activeTab === 'productDetails' && selectedProduct && (
          <ProductDetails product={selectedProduct} onAddToCart={handleAddToCart} onBack={handleBackToBrowse} />
        )}
