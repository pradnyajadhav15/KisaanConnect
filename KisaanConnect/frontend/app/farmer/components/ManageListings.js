...
export default function ManageListings({ crops, onEditCrop, onDeleteCrop }) {
  ...
  const handleEditClick = useCallback((crop) => {
    ...
    setEditForm({
      ...
      unit: crop.unit || 'kg',
      price_per_unit: crop.price_per_unit != null ? crop.price_per_unit : (crop.price || ''),
            ...
            <div key={crop.id} className="crop-item">
              {editingId === crop.id ? (
                <form className="edit-form" onSubmit={handleEditSubmit} noValidate>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      <label htmlFor={'price-' + crop.id}>{'Price (Rs. per ' + editForm.unit + ') *'}</label>
                ...
                <>
                  ...
                  <div className="crop-actions">
                    <button className="edit-button" onClick={() => handleEditClick(crop)}>Edit</button>
                    <button className="delete-button" onClick={() => setConfirmDelete(crop.id)}>Delete</button>
                  </div>
                </>
              )}
            </div>
