...
/**
 * Web browser implementation of `debug()`.
 */
...
/**
 * Detect whether browser console supports colors
 */
function useColors() {
  ...
  return (
    (typeof document !== 'undefined' && document.documentElement?.style?.WebkitAppearance) ||
...
/**
 * Format console log arguments with namespace and color
 */
function formatArgs(args) {
  ...
  // Find last %c in string to insert our CSS
  let index = 0;
  let lastC = 0;
...
/**
 * Use console.debug() if available, fallback to console.log, else no-op
 */
...
function save(namespaces) {
  try {
    ...
    // Swallow errors if localStorage is inaccessible
  }
}

/**
 * Load `namespaces` from localStorage or process.env.DEBUG in Electron
 */
function load() {
  let r;
  try { r = exports.storage.getItem('debug'); } catch (err) {}
  if (!r && typeof process !== 'undefined' && 'env' in process) r = process.env.DEBUG;
  return r;
}

/**
 * Safe access to localStorage, returns undefined if not available
 */
function localstorage() {
  try { return localStorage; } catch (err) { /* ignore */ }
}

// Attach browser exports to common debug logic
...
/**
 * Add %j formatter for JSON stringification
 */
formatters.j = function (v) {
  try { return JSON.stringify(v); }
  catch (err) { return '[UnexpectedJSONParseError]: ' + err.message; }
};
