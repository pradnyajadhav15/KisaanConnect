...
// --- Field accessors: backend returns price_per_unit / image_url and no
// --- farmer name. These helpers tolerate both the backend names and any
// --- pre-mapped names, so the component works regardless of the fetch layer.
const getPrice  = (p) => Number(p.price_per_unit ?? p.price ?? 0);
    ...
    <div className="browse-products">
      ...
      <div className="search-filter-container">
        ...
        <div className="filters-container">
          ...
          {[
            { id: 'minPrice', label: 'Min Price (₹)', placeholder: 'Min' },
            { id: 'maxPrice', label: 'Max Price (₹)', placeholder: 'Max' },
              ...
              return (
                <div key={product.id} className="product-card">
                  ...
                  <div className="product-info">
                    ...
                    <p className="product-price">₹{price} per {product.unit}</p>
                    <p className="product-quantity">Available: {product.quantity} {product.unit}</p>
                    {farmer && (
                      <p className="product-farmer">Farmer: {farmer}</p>
                    )}
                    {product.location && (
