...
const PROBLEM_ICONS = { disease: '🦠', pest: '🐛', nutrient: '🍂', water: '💧' };

// --------------------------------------------------
// Parse Gemini response text → structured result
// --------------------------------------------------
...
const CropHealthAnalysis = () => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      // Send to OUR backend, which holds the Gemini key. Key never reaches the browser.
          ...
          <div className={`analysis-result ${analysisResult.status}`}>
            ...
            {analysisResult.status === 'healthy' ? (
              ...
              <div className="unhealthy-result">
                  ...
                  {analysisResult.problems.map((problem, i) => (
                    <div key={i} className="problem-item">
                      <div className="problem-header">
                        <span className="problem-icon">{PROBLEM_ICONS[problem.type] || '❓'}</span>
                        <h5>{problem.name}</h5>
                      </div>
                ...
                <p className="seek-advice">For specific advice, consult a local agricultural expert.</p>
              </div>
            )}
          </div>
        )}
